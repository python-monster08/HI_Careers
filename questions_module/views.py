...
def generate_questions_document(request):
    ...
    return HttpResponse(f"Document created at {file_path}", content_type='text/plain')
...
def generate_questions_csv_view(request):
    ...
    with open(file_path, mode='w', newline='', encoding='utf-8') as file:
        ...
        # Write the header
        writer.writerow(['SrNo.', 'Question', 'Option a', 'Option b', 'Option c', 'Option d', 'Correct Option(a/b/c/d)', 'Keyword'])

        for index, question in enumerate(Question.objects.all(), start=1):
            # Assuming 'Keyword' or similar information is stored in the 'other' field
            keywords = question.other  # This is an example; adjust as needed
            ...
            writer.writerow([
                ...
                keywords or ''  # Keyword, adjust this according to where keywords are stored
            ])
    
    # Return an appropriate response
    # For example, you could return the path of the generated file:
    return HttpResponse(f"CSV file has been successfully generated at: {file_path}")
